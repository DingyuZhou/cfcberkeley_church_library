...
DECLARE v_lent_item_id BIGINT := NULL;
...
BEGIN
  ...
  SELECT i.id, i.borrower_id INTO v_lent_item_id, v_borrower_id FROM item AS i
  ...
  IF v_borrower_id IS NULL THEN
    SELECT
      'ERROR' AS status,
      CASE
        WHEN v_lent_item_id IS NULL THEN 'Fail to find the book.'
